...
def clust_rank(mat, initial_rank=None, distance='cosine', labeled=None, mask=None, level=100):
    s = mat.shape[0]
    ...
    if labeled is not None:
        ...
        labeled[~mask.astype(bool)] = -101
        
        # print(len(np.unique(labeled[mask.astype(bool)])))
        label_ref = labeled[mask.astype(bool)]
        index = np.arange(len(labeled))[mask.astype(bool)]

        orig_dist_lb = metrics.pairwise.pairwise_distances(mat[mask.astype(bool)], mat[mask.astype(bool)], metric=distance)
...
def req_numclust(c, data, req_clust, distance, labeled=None, mask=None):
    ...
    for i in range(iter_):
        adj, orig_dist = clust_rank(mat, initial_rank=None, distance=distance, labeled=new_label, mask=new_mask)
...
def get_new_label(y_true, y_pred, mask):
    y_true_lb = y_true[mask.astype(bool)].astype(int)
    y_pred_lb = y_pred[mask.astype(bool)].astype(int)
    ...
    # y_pred_unlb = y_pred[~mask.astype(bool)].astype(int)
...
def SNC(data, initial_rank=None, req_clust=None, distance='cosine', ensure_early_exit=True, verbose=True, labeled=None, mask=None):
    """ 
    Thuật toán Selective Neighbor Clustering (SNC) - Phiên bản Bán giám sát (Semi-supervised).
    Được kế thừa từ thuật toán FINCH (First Integer Neighbor Clustering Hierarchy).
    
    Giải thích nguyên lý hoạt động (Dành cho báo cáo):
    1. Thuật toán không yêu cầu phải biết trước số cụm ở giai đoạn đầu, nó tự động gom các điểm gần nhất (First Neighbor) thành các cụm nhỏ.
    2. Trong quá trình gom cụm, nếu có tham số `labeled` (các Anchors), mô hình sẽ sử dụng chúng như lực hấp dẫn:
       - Các điểm có cùng nhãn sẽ bị ép hút lại với nhau.
       - Các điểm khác nhãn (hoặc giữa Labeled và Unlabeled) sẽ được gán khoảng cách vô cực (Cannot-link constraints) để ép chúng tách rời.
    3. Thuật toán liên tục gộp các cụm nhỏ thành cụm lớn cho đến khi đạt được số lượng cụm yêu cầu (`req_clust` = 100).
    """
    # Cast input data to float32
    data = data.astype(np.float32)

    min_sim = None
    adj, orig_dist = clust_rank(data, initial_rank, distance, labeled=labeled, mask=mask, level=0)
    initial_rank = None
    group, num_clust = get_clust(adj, [], min_sim)
    c, mat = get_merge([], group, data)

    if labeled is not None:
        new_label, new_mask = get_new_label(labeled, c, mask)
    else:
        new_label, new_mask = None, None
    ...
    while exit_clust > 1:
        adj, orig_dist = clust_rank(mat, initial_rank, distance, labeled=new_label, mask=new_mask, level=k)
    ...
    if req_clust is not None:
        ...
        #     req_c, d_all = req_numclust(c[:, ind[-1]], data, req_clust, distance, labeled=labeled, mask=mask)
        # else:
    ...
    return c, num_clust, req_c, d_all
